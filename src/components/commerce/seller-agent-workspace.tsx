...
export function SellerAgentWorkspace({ examples, initialData }: SellerAgentWorkspaceProps) {
  ...
  useEffect(() => {
    const syncAuditEntries = () => {
      setAuditEntries(readSellerListingMutationStore().auditLog);
    };

    syncAuditEntries();
    window.addEventListener(sellerListingMutationUpdatedEvent, syncAuditEntries);

    return () => {
      window.removeEventListener(sellerListingMutationUpdatedEvent, syncAuditEntries);
    };
  }, []);
  ...
  async function applyDraftPreview() {
    const draftPreview = data.draftPreview;

    if (!draftPreview) {
      setApplyState({
        message: "Uygulanacak listing taslağı bulunamadı.",
        status: "error",
      });
      return;
    }
  ...
  function rollbackAuditEntry(auditId: string) {
    const result = rollbackSellerListingMutation(auditId);
    setAuditEntries(readSellerListingMutationStore().auditLog);
      ...
      >
        <div>
          ...
          {data.draftPreview ? (
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              <ListingSnapshot label="Önce" tone="light" values={data.draftPreview.beforeListing} />
              <ListingSnapshot label="Sonra" tone="dark" values={data.draftPreview.afterListing} />
            </div>
          ) : null}
        </div>
        ...
        {data.draftPreview ? (
          <ListingMutationApprovalPanel
            ...
            onApply={() => void applyDraftPreview()}
            onRollback={rollbackAuditEntry}
          />
        ) : null}
      </section>
    </div>
